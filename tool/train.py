...
def profile(model, args):
    ...
    N = 1024 
    C_in = 0 
    coord = torch.randn(B * N, 3).cuda(non_blocking=True)
    feat = torch.randn(B * N, C_in).cuda(non_blocking=True)
    ...
    input = [coord, feat, offsets]

    logger.info(f'test input size: ({coord.shape, feat.shape})') 
    detailed = False
    ...
    logger.info(f'{flops/(float(B)*1e9): .2f}\t{macs/(float(B)*1e9): .2f}\t{params/1e6: .3f}')
...
def main():
    ...
    if args.manual_seed is None: 
        args.manual_seed = int(np.random.randint(10000, size=1)[0])
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    if args.class_weight:
        logger.info('Use class weight ! ')
        weight = train_data.get_class_weight() 
        weight = torch.FloatTensor(weight)
    else:
        weight = None 
    criterion = nn.CrossEntropyLoss(ignore_index=args.ignore_label, weight=weight).cuda()

    val_loader = None
    if args.evaluate:
        val_transform = None
        val_data = DSNetDataset(
